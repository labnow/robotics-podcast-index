...
def upsert_episode(conn: sqlite3.Connection, ep: dict, keyword: str | None,
    ...
    if audio_url:
        existing_source = conn.execute("SELECT episode_id FROM episode_sources WHERE audio_url=? LIMIT 1", (audio_url,)).fetchone()
    if existing_source is None and rss_guid:
        existing_source = conn.execute("SELECT episode_id FROM episode_sources WHERE rss_guid=? LIMIT 1", (rss_guid,)).fetchone()
    if existing_source is None and title and published:
        existing_source = conn.execute("""SELECT episode_id FROM episodes
          WHERE title=? AND podcast_name=? AND substr(published_at,1,10)=substr(?,1,10)
          LIMIT 1""", (title, podcast.get("title") or ep.get("podcastTitle"), published)).fetchone()
    if existing_source:
        eid = existing_source[0]
    ...
    conn.execute(
        ...
        duration_seconds,play_count,comment_count,url,raw_json) VALUES(?,?,?,?,?,?,?,?,?,?,?)
    ...
    conn.execute("""INSERT INTO episode_sources
      (episode_id,source,source_episode_id,source_url,audio_url,feed_url,rss_guid,transcript_url)
      ...
      rss_guid=excluded.rss_guid,transcript_url=excluded.transcript_url,updated_at=CURRENT_TIMESTAMP""",
      (eid, source, source_id, ep.get("url"), audio_url, ep.get("feedUrl"), ep.get("rssGuid"), ep.get("transcriptUrl")))
